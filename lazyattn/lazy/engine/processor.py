...
def process_inputs(
    ...
    # NOTE(Haocheng): for lazy attention, we pad the document sequence to be a 
    ...
    if document_seq is not None:
        ...
        for doc in document_seq:
            ...
            num_padding_tokens.append(pad_length)

            if pad_length > 0:
                try:
                    pad_token = self.tokenizer.tokenizer.pad_token
                except AttributeError:
                    logging.warning("Warning: AttributeError - tokenizer.pad_token, use '<pad>'.")
                    pad_token = "<pad>"
                if pad_token is None:
                    logging.warning("Warning: tokenizer.pad_token is None, use '<pad>'.")
                    pad_token = "<pad>"
                ...
                if len(pad_token_ids) > 1:
                    logging.warning(f"Warning: the pad token id is not a single token. {pad_token_ids}")
