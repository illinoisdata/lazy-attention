    ...
    def generate(
        ...
        for i in range(params.max_new_tokens):
            ...
            if past_key_values is None:
                ...
                start.record()
                out = self.lm(input_ids=input_ids, position_ids=position_ids, past_key_values=cache, use_cache=True)
                end.record()
                ...
                if use_full_position_ids:
                    position_ids = torch.tensor(
                        [position_ids_og + list(range(position_offset, position_offset + i))], device=device, dtype=torch.long
                    )
                ...
                start.record()
                out = self.lm(input_ids=input_ids, position_ids=position_ids, past_key_values=past_key_values, use_cache=True)
                end.record()
                torch.cuda.synchronize()
                inference_time += start.elapsed_time(end)
