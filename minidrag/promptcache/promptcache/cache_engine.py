...
from .schema import Module, Parameter, Path, Schema, TokenSequence, UnionModule
...
class TokenSequenceCache:
    ...
    def upload(self, device: str):
        if self.device_cache is None:
            self.device_cache = [
                (kv[0].to(device, non_blocking=True), kv[1].to(device, non_blocking=True)) for kv in self.host_cache
            ]
...
class PromptCache:
    ...
    def __init__(self, max_ctx_length: int, num_layers: int, num_head: int, head_dim: int, target_device: torch.device):
        ...
        self.device_cache = [
            (
                ...
                torch.empty(num_head, max_ctx_length, head_dim, device=target_device, dtype=torch.half),
            )
    ...
    def cache(self) -> KVCache:
        return [
            (self.device_cache[i][0][:, : self.length, :], self.device_cache[i][1][:, : self.length, :])
            for i in range(len(self.device_cache))
        ]
...
class SchemaCache:
    ...
    def get_cache_l2(
        self, seq1: TokenSequence, seq2: TokenSequence
    ) -> Optional[Tuple[TokenSequenceCache, TokenSequenceCache]]:
...
class CacheEngine:
    ...
    def add_schema(
        self, schema: Union[str, Schema], batch_size: int = 1, max_tokens: Optional[int] = None, no_cache: bool = False
    ):
        ...
        self.schemas[schema.name] = SchemaCache(
            schema, self.lm, batch_size, target_device=self.target_device, no_cache=no_cache
        )
    ...
    def process(
        self, prompt: Prompt, no_cache: bool = False, return_full_position_ids: bool = False
    ) -> Tuple[List[int], List[int], float, Optional[KVCache]]:
