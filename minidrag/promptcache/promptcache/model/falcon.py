...
class FalconFlashAttention2(FalconAttention):
    ...
    def forward(
        ...
        query_layer = query_layer.reshape(batch_size, self.num_heads, -1, self.head_dim).transpose(1, 2).to(torch_dtype)
...
class FalconDecoderLayer(nn.Module):
    def __init__(self, config: FalconConfig):
        ...
        self.self_attention = (
            FalconAttention(config) if not getattr(config, "_flash_attn_2_enabled", False) else FalconFlashAttention2(config)
        )
