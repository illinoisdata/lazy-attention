...
class LazyGPUModelRunner(GPUModelRunner):
    ...
    def __init__(
        ...
        # We add three fields, is_lazy_req, lazy_offset, and lazy_mask.
        ...
        self.lazy_mask = torch.full((self.max_num_reqs,
                                     self.max_num_blocks_per_req),
                                     self.block_size,
                                     dtype=torch.int32,
                                     device=self.device)
        self.lazy_mask_cpu = torch.full((self.max_num_reqs, 
                                         self.max_num_blocks_per_req),
                                         self.block_size,
                                         dtype=torch.int32,
                                         device="cpu",
                                         pin_memory=self.pin_memory)
        self.lazy_mask_np = self.lazy_offset_cpu.numpy()
...
# /////////////////////////////////////////////////////////////////////////////
                ...
                is_lazy=new_req_data.is_lazy_req,
                lazy_mask=new_req_data.lazy_mask,
                lazy_offset=new_req_data.lazy_offset,
# /////////////////////////////////////////////////////////////////////////////
            )
...
# /////////////////////////////////////////////////////////////////////////////
        ...
        # Flush the row in the lazy_mask and lazy_offset
        self.is_lazy_req_cpu[:num_reqs].fill_(False)
        self.lazy_mask_cpu[:num_reqs].fill_(self.block_size)
        self.lazy_offset_cpu[:num_reqs].fill_(0)
        for idx, req_id in enumerate(req_ids):
            self.is_lazy_req_cpu[idx] = self.requests[req_id].is_lazy
            if self.requests[req_id].is_lazy:
                self.lazy_offset_np[idx, 
                    :len(self.requests[req_id].lazy_offset)] = self.requests[req_id].lazy_offset
                self.lazy_mask_np[idx,
                    :len(self.requests[req_id].lazy_mask)] = self.requests[req_id].lazy_mask
...
# ///////////////////////////////////////////////////////////////////////////
                ...
                attn_metadata[layer_name].is_lazy_req = self.is_lazy_req[:num_reqs]
                attn_metadata[layer_name].lazy_mask = self.lazy_mask[:num_reqs]
                attn_metadata[layer_name].lazy_offset = self.lazy_offset[:num_reqs]         
# ////////////////////////////////////////////////////////////////////////////
