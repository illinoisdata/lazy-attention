...
class DragKVCacheManager(KVCacheManager):
    ...
    def get_computed_blocks_docs(
        ...
        # TODO(haocheng): support async load via KVConnector
        # # Request already has blocks from async load via KVConnector.
        # num_existing_blocks = len(
        #     self.single_type_manager.req_to_blocks[request.request_id])
        # if num_existing_blocks > 0:
        ...
        if (not self.enable_caching
                or request.sampling_params.prompt_logprobs is not None):
            return [KVCacheBlocks.create_empty() for _ in range(num_docs)], \
        ...
        for doc_idx in range(num_docs):
            ...
            computed_blocks_docs[doc_idx] = (
                self.single_type_manager.find_longest_cache_hit(block_hashes_doc))
        ...
        return [KVCacheBlocks(computed_blocks_doc) for computed_blocks_doc in computed_blocks_docs],\
    ...
    def allocate_slots(
        ...
        # TODO(haocheng): consider the impact of doc to max blk
